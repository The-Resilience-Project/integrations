...
if ($method === 'POST') {
    ...
    log_info('v2 School conference prize pack request started', [
        'endpoint' => 'v2/schools/conference-prize-pack',
    ...
    try {
        ...
        log_info('v2 School conference prize pack processed', ['status' => $result ? 'success' : 'fail']);
        send_response(['status' => $result ? 'success' : 'fail']);
    } catch (Exception $e) {
        log_exception($e, ['endpoint' => 'v2/schools/conference-prize-pack']);
        send_response([
            'status' => 'fail',
            'message' => 'Error processing school conference prize pack: '.$e->getMessage(),
        ]);
    }
}
